...
wdetecta (cite) and manually curated in clampfit 
(hence the use of pyabf)
...
fname = "abf/m648_f7_r2dA_curated2.abf"
abf = pyabf.ABF(fname)
...
for stmp in abf.sweepList:
	...
	sweepMat[:,stmp]=sy
	ymaxpre[stmp]=max(sy[preind1:preind2+34])
	yminpre[stmp]=min(sy[preind1:preind2+34])
...
mmax = np.argmax(msweep[preind2:preind2+8])+preind2

mpost = msweep[mmax:mmax+32]
xpost = sx[mmax:mmax+32]-tstart
